...
class APymongodb:
    ...
    def create_project_db_from_csv(self):
        ...
        with open('database/calendar.csv', newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                #print("type",type(row), "data", row)
                cal_coll.insert_one(dict(row))
        with open('database/listings.csv', newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                #print("type",type(row), "data", row)
                list_coll.insert_one(dict(row))
        with open('database/reviews.csv', newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                #print("type",type(row), "data", row)
